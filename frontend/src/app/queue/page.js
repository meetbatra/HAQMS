...
export default function QueueMonitor() {
  ...
  useEffect(() => {
    ...
    const fetchQueueData = async () => {
      try {
        const res = await fetch(`${API_BASE_URL}/queue`, { signal: controller.signal });
        if (!res.ok) {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-6 sm:p-8">
        ...
        <div className="glass border-t border-l border-white/40 shadow-xl backdrop-blur-md p-6 sm:p-8 rounded-2xl shadow-lg border border-slate-200 dark:border-slate-800 mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            <span className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-blue-500/15 text-blue-600 dark:text-blue-400 text-xs font-bold uppercase tracking-wide border border-blue-500/20">
              <RefreshCw className="h-3.5 w-3.5 animate-spin" />
              Auto Refreshing
            </span>
            <div className="p-2 bg-slate-100 dark:bg-slate-800 rounded-lg text-slate-600 text-xs font-mono">
              Polls: {refreshCount}
            </div>
          </div>
        </div>
        ...
        {loading && tokens.length === 0 ? (
          ...
          <div className="glass border-t border-l border-white/40 shadow-xl backdrop-blur-md p-12 text-center rounded-2xl border border-dashed border-slate-200 dark:border-slate-800">
          ...
          <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
            {Object.entries(groupedTokens).map(([docId, docInfo]) => (
              <div
                key={docId}
                className="glass border-t border-l border-white/40 shadow-xl backdrop-blur-md rounded-2xl shadow-lg border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col h-full hover:shadow-blue-500/5 hover:border-blue-500/30 transition-all duration-300"
              >
                {/* Doctor Title Header */}
                ...
                <div className="p-6 flex-1 flex flex-col justify-between">
                  ...
                  <div className="mb-6">
                    ...
                    {docInfo.calling ? (
                      <div className="bg-blue-500/10 dark:bg-blue-500/5 border border-blue-500/30 p-6 rounded-2xl text-center shadow-inner relative overflow-hidden group">
                        ...
                        <span className="block text-5xl font-black text-blue-600 dark:text-blue-400 tracking-wider animate-pulse">
                      ...
                      <div className="bg-slate-100 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-800/80 p-6 rounded-2xl text-center shadow-inner">
                        <span className="block text-2xl font-extrabold text-slate-600 dark:text-slate-400 tracking-wider italic">
                          Idle
                        </span>
